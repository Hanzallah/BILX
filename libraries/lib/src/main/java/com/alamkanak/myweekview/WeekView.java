...
import static com.alamkanak.myweekview.WeekViewUtil.*;
...
public class WeekView extends View {
    ...
    //To draw a rectangle
    float firstHourBegin;
    float firstHourEnd;
    float secondHourBegin;
    float secondHourEnd;
    float thirdHourBegin;
    float columnBegin;
    float columnEnd;
    float hourWidth;
    float stroke;
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public void onLongPress(MotionEvent e) {
            ...
            firstHourBegin = 140 / (float) 1080 * getWidth();
            firstHourEnd = 440 / (float) 1080 * getWidth();
            secondHourBegin = 460 / (float) 1080 * getWidth();
            secondHourEnd = 760 / (float) 1080 * getWidth();
            thirdHourBegin = 780 / (float) 1080 * getWidth();
            columnBegin = 110 / (float) 1920 * getHeight();
            columnEnd = getHeight();
            hourWidth = 300 / (float)1080 * getWidth();
            stroke = 13 / (float)1080 * getWidth();
            gap = 0;

            if ( (( x >= firstHourBegin && x <= firstHourEnd) || ( x >= secondHourBegin && x <= secondHourEnd)
                    || ( x >= thirdHourBegin)) && ( y >= columnBegin && y <= columnEnd)  ) {
                longPressed = true;

                if (x > firstHourEnd && x <= secondHourEnd)
                    gap = 20 / (float) 1080 * getWidth();
                else if (x > secondHourEnd)
                    gap = 40 / (float) 1080 * getWidth();

                //set the coordinates of the rectangle showing the selected hours
                beginCoordinate.x = x - ((x - firstHourBegin - gap) % hourWidth) + 7 / (float) 1080 * getWidth();;
                beginCoordinate.y = y;
                endCoordinate.y = beginCoordinate.y;
                endCoordinate.x = beginCoordinate.x + hourWidth - stroke;
                startTime = getTimeFromPoint(x, y);
                Log.i("Tag", "Down on: " + ( (Integer) (startTime.get(Calendar.HOUR_OF_DAY))).toString());
                invalidate();
            }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        Calendar today = today(); if(today == null) today = new java.util.GregorianCalendar(2000, 1, 1);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (longPressed) {
            ...
            if (action == MotionEvent.ACTION_MOVE) {
                if ( Y >= columnBegin && Y <= columnEnd )
                    endCoordinate.y = Y;
                Log.i("TAG", "moving: (" + X + ", " + Y + ")");
                invalidate();
            }
            else if (action == MotionEvent.ACTION_UP ){
                endTime = getTimeFromPoint( x, y);
                Log.i("TAG", "Up on: " + ((Integer) endTime.get(Calendar.HOUR_OF_DAY)).toString());
                longPressed = false;
                isReleased = true;
                invalidate();
            }
